# I have created this file -  Ajeyata 
...
def index(request):
    return HttpResponse("<h2>Hello Ajeyata</h2>")
...
def charcount(request):
    return HttpResponse("char count <br> <a href='/'> BACK TO HOME </a>")
...
#using GET and POST
def removepunc(request):
    djtext=request.POST.get('text', 'default')  #getting the text from the user
    print(djtext)
    #analyzing the text
...
def analyze(request):
    ...
    if (extraspaceremover=="on"):
        analyzed =""
        for index, char in enumerate (djtext):   #enumrate iterate the blank index spaces
